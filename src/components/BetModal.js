...
import { setBetOpen } from "../store/modals/modalSlice";
import Bet from "./history/Bet";

const BetModal = () => {
  const { betOpen } = useSelector((state) => state.modals);
  ...
  return (
    <Transition appear show={betOpen} as={Fragment}>
      <Dialog
        as="div"
            ...
            <div className="inline-block w-full max-w-xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-theme border border-rose-300 shadow-xl rounded-xl">
              ...
              >
                <p
                  ...
                  tabIndex="0"
                >
                  <i className="fa-solid fa-chess-rook mr-2 py-1.5 px-2 rounded-full bg-green-light-200 text-dark-green-500"></i>
                  Bet Details
                </p>
                <i
