...
import { setTwoFaOpen } from "../store/modals/modalSlice";
import TwoFactor from "./profile/TwoFactor";

const AuthenticatorModal = () => {
  const { twoFaOpen } = useSelector((state) => state.modals);
  const dispatch = useDispatch();

  const closeModal = () => {
    dispatch(setTwoFaOpen(false));
  };

  return (
    <Transition appear show={twoFaOpen} as={Fragment}>
      <Dialog
        as="div"
