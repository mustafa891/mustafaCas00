...
import { setPasswordOpen } from "../store/modals/modalSlice";
import ChangePassword from "./profile/ChangePassword";

const ChangePasswordModal = () => {
  const { passwordOpen } = useSelector((state) => state.modals);
  const dispatch = useDispatch();

  const closeModal = () => {
    dispatch(setPasswordOpen(false));
  };

  return (
    <Transition appear show={passwordOpen} as={Fragment}>
      <Dialog
        as="div"
